...
To add missing data and process your data further into a format suitable for spatial ecological analyses, keep on reading.  
1. Open the script **"data-raw/complete_database.R"** that contains functions to add missing data to the database. These functions either use data from secondary sources or calculate new values based on some assumptions. There are basically two options: 1) You simply run the full script as it is. If you do, please make sure you are aware of these sources and assumptions so you understand how to interpret the data. 2) You use the provided functions by the package, but maybe not all, or in another order, or you alter them somewhat to fit your own secondary sources and assumptions you want to use. Keep reading to understand what you can add and calculate.    
2. If start and end coordinates are provided for the TripleD tracks, the midpoint and track length can be calculated.
Missing depth values for sampling stations can be obtained from bathymetry data using track midpoints.
