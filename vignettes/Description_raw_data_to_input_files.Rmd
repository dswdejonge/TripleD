...
## isBulkWeight
NICO10 cruise (64PE438): If at one sampling station multiple entries of one species exists or if there is one entry with a count > 1, isBulkWeight is set to 1 for all entries (so also for the species with a WetWeight of NA). isBulkWeight was set to 0 if there was only one individual of that species in the sample or if there was not wet weight measured at all.  
test_Lavaleye: NA.  
RWS2018: NA (no wet weights reported).  
RWS2019: Most of the reported wet weights were sample bulk weights, so isBulkWeight = 1. This was changed to isBulkWeight = 0 if only one individual was reported, or if no weight was reported at all.  

## BulkWeightType
NICO10 cruise (64PE438): Was set to 'NA' if isBulkWeight was 0. Was set to 'Sample' for all other entries, and changed manually to 'Entry' when the biomass only concerned one entry.  
RWS2018: NA (no wet weights reported).  
...
## isBulkWeightAFDW
NICO10 cruise (64PE438): NA.  
RWS2018: Was set using the count column using Excel: IF(B1 = 1, 0, 1).  
RWS2019: NA (no AFDW reported).  
