...
The database contains `r length(unique(stations$StationID))` stations sampled during `r length(unique(stations$CruiseID))` cruises. The oldest sample was taken on `r min(stations$Date)` and the most recent sample was taken on `r max(stations$Date)`.
...
lats <- c(min(stations$Lat_DD), max(stations$Lat_DD))
lons <- c(min(stations$Lon_DD), max(stations$Lon_DD))
